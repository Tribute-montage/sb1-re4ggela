...
do $$
begin
  drop policy if exists "Editors and clients can access drafts" on public.order_drafts;
  drop policy if exists "Users can manage feedback" on public.draft_feedback;
...
create policy "Draft access and management"
...
create policy "Feedback access and management"
  ...
  using (
    ...
    exists (
      ...
      and (
        order_drafts.editor_id = auth.uid() or
        exists (
          select 1 from public.orders
          where orders.id = order_drafts.order_id
          and orders.client_id = auth.uid()
